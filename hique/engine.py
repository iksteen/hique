...
class Engine:
    ...
    @overload
    async def execute(
        self, query: Union[ModelSelectQuery[T_Model], InsertQuery[T_Model]]
    ) -> List[T_Model]:
        ...
    ...
    async def execute(
        ...
        try:
            async with self.state.lock:
                ...
                if isinstance(query, (BaseSelectQuery, InsertQuery)):
                    return query.unwrap(engine=self, rows=result)
                return result
        finally:
            if release:
                await conn.release()
